...
namespace GameLauncher
{
    ...
    public partial class MainWindow : Window
    {
        ...
        // Close Launcher after opening the App? // Now managed by launcher settings
        //bool CloseLauncherAfterOpening = false;

        // Allow multiple instances of the Launcher?
        bool AllowMultiplesInstances = false;

        // Update after checking for updates? // Now managed by every game settings
        ...
        public void InitializeLauncher ()
        {
            ...
            // Set Game List Main URL
            GAMES_LIST_URL = HOST_URL + $"Config/GameList.txt";
        ...
        private void MainWindow_Loaded (object sender, RoutedEventArgs e)
        {
            ...
            Icon_Blur.Opacity = 0;
            Blur.Opacity = 0;
            Game_Library_Content.Opacity = 0;
            Game_Library_Content.Visibility = Visibility.Visible;

            // Load All Games Control
        ...
        /// <summary>
        /// Switch the current launcher view. 
        /// "Game"to display the selected game.
        ...
        public async void SwitchTab (string newTab)
        {
            ...
            {
                case "Game":
                    currentWindowTab = "Game";
                    AllGamesControl.IsSelected = false;
                    AllGamesControl.IsEnabled = false;
                ...
                case "Library":
                    ...
                    AllGamesControl.IsSelected = true;
                    AllGamesControl.IsEnabled = false;

                    // Unselect all GameIcons
                    foreach (GameIconControl gc in gameIconControls)
                    {
                        gc.IsSelected = false;
                    }
        ...
        private async void FetchGamesFromHost ()
        {
            ...
            if (!string.IsNullOrEmpty (content))
            {
                ...
                {
                    gamesListToLoad = gameListHost.Games;
                }
                else // If we cannot get the game list from host, then use the local one
                {
            ...
            // Load games
            foreach (GameInfo gameInfo in gamesListToLoad)
            {

                // Hide games that are not supposed to be shown
                if (gameInfo.Show == false)
                {
                    continue;
                }

                // Load saved game info from local DB
                if (gameListLocal != null && gameListLocal.Count > 0)
                {
                    ...
                    {
                        // Load Launcher Info
                        gameInfo.IsInstalled = gameInfoLocal.IsInstalled;
                        gameInfo.InstallDate = gameInfoLocal.InstallDate;
                        gameInfo.LastPlayedDate = gameInfoLocal.LastPlayedDate;
                        gameInfo.InstallPath = gameInfoLocal.InstallPath;
                        gameInfo.AutomaticUpdates = gameInfoLocal.AutomaticUpdates;
            ...
            // Check if we added at least 1 game
        ...
        public async void SelectGame (GameInfo gameInfo)
        {
            ...
            // Set GameIcon as selected
            foreach (GameIconControl gc in gameIconControls)
            {
                gc.IsSelected = gc.GameInfo.Title == gameInfo.Title;
            }
        ...
        public void SearchFieldTextChanged (string searchText = "")
        {
            ...
            foreach (var gameCover in gameCovers)
            {
                ...
                {
                    gameCover.Visibility = Visibility.Collapsed;
                }
            }
        }
                ...
                foreach (var gameCover in gameCovers)
                {
                    switch (category)
                    {
                        case Category.FreeToPlay:
                            gameCover.Visibility = gameCover.GameInfo.IsFreeToPlay == true ? Visibility.Visible : Visibility.Collapsed;

                            break;
                        case Category.Multiplayer:
                            gameCover.Visibility = gameCover.GameInfo.IsMultiplayer == true ? Visibility.Visible : Visibility.Collapsed;

                            break;
                        case Category.MacOS:
                            gameCover.Visibility = gameCover.GameInfo.IsMacOS == true ? Visibility.Visible : Visibility.Collapsed;

                            break;
                        case Category.Mobile:
                            gameCover.Visibility = gameCover.GameInfo.IsMobile == true ? Visibility.Visible : Visibility.Collapsed;
                            break;
                        case Category.Installed:
                            gameCover.Visibility = gameCover.GameInfo.IsInstalled ? Visibility.Visible : Visibility.Collapsed;
                            break;
                        case Category.Favorites:
                            gameCover.Visibility = gameCover.GameInfo.IsFavorite ? Visibility.Visible : Visibility.Collapsed;
                            break;
                        default:
                            gameCover.Visibility = Visibility.Collapsed;
                            break;
